...
void LocalNotifierPlugin::Notify(
  ...
  // Upstream 0.1.6 serialises `silent` from Dart but never reads it here, so
  // every toast played the default notification sound. Honour it instead.
  bool silent = false;
  auto silent_it = args.find(flutter::EncodableValue("silent"));
  if (silent_it != args.end()) {
    if (const bool* value = std::get_if<bool>(&silent_it->second)) {
      silent = *value;
    }
  }
